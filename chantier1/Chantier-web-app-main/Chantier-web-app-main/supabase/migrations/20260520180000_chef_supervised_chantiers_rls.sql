/*
  Chef d'équipe : lire affectations et déclarations de tous les ouvriers
  sur les chantiers où au moins une affectation a chef_equipe_id = ce chef.
*/
...
CREATE POLICY "Chefs can view affectations on supervised chantiers"
  ...
  USING (
    EXISTS (
      SELECT 1
      FROM affectations_chantiers supervisor
      WHERE supervisor.chantier_id = affectations_chantiers.chantier_id
        AND supervisor.chef_equipe_id = auth.uid()
        AND supervisor.date_fin IS NULL
    )
  );
...
CREATE POLICY "Chefs can view declarations on supervised chantiers"
  ...
  USING (
    EXISTS (
      ...
      WHERE a.chantier_id = declarations_heures.chantier_id
...
CREATE POLICY "Chefs can view periods on supervised chantiers"
  ...
  USING (
    EXISTS (
      SELECT 1
      FROM affectations_chantiers a
      WHERE a.chantier_id = periodes_travail.chantier_id
        AND a.chef_equipe_id = auth.uid()
        AND a.date_fin IS NULL
    )
  );
...
CREATE POLICY "Chefs can update periods on supervised chantiers"
  ...
  TO authenticated
  USING (
    statut IN ('terminee', 'validee', 'en_cours')
    AND EXISTS (
      SELECT 1
      FROM affectations_chantiers a
      WHERE a.chantier_id = periodes_travail.chantier_id
