/*
  Heures normales / supplémentaires selon le cadre horaire du chantier :
  - normales = intersection période ouvrier ∩ [chantier.heure_debut, chantier.heure_fin]
  - supplémentaires = uniquement le travail après chantier.heure_fin
  Sans cadre chantier : repli 7h / au-delà (comportement historique).
*/
...
RETURNS TABLE (
  heures_normales decimal(6, 2),
  heures_supplementaires decimal(6, 2),
...
BEGIN
  ...
  w0 := minutes_from_time(travail_debut);
  w1 := minutes_from_time(travail_fin);
  total_min := w1 - w0;
  total_dec := ROUND((total_min::decimal / 60), 2);

  IF cadre_debut IS NULL OR cadre_fin IS NULL OR cadre_fin <= cadre_debut THEN
    heures_normales := LEAST(total_dec, 7.0);
    heures_supplementaires := GREATEST(total_dec - 7.0, 0.0);
    total_heures := total_dec;
    RETURN NEXT;
    RETURN;
  END IF;

  c0 := minutes_from_time(cadre_debut);
  c1 := minutes_from_time(cadre_fin);

  norm_min := GREATEST(0, LEAST(w1, c1) - GREATEST(w0, c0));
  supp_min := CASE WHEN w1 > c1 THEN w1 - GREATEST(w0, c1) ELSE 0 END;

  heures_normales := ROUND((norm_min::decimal / 60), 2);
  heures_supplementaires := ROUND((supp_min::decimal / 60), 2);
  total_heures := total_dec;
  RETURN NEXT;
END;
$$;
...
SELECT
  ...
  CASE WHEN BOOL_OR(pt.panier_repas) THEN 1 ELSE 0 END AS nb_paniers,
  CASE WHEN BOOL_OR(pt.deplacement) THEN 1 ELSE 0 END AS nb_deplacements,
  CASE
