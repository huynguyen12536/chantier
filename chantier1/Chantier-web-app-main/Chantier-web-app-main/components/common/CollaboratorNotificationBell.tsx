...
import { Bell, Building2, Check, Clock, X } from 'lucide-react-native';
import { Colors } from '@/constants/colors';
import {
  useCollaboratorNotifications,
  type CollaboratorNotification,
  type CollaboratorNotificationKind,
} from '@/contexts/CollaboratorNotificationsContext';
...
function RowIcon({ kind }: { kind: CollaboratorNotificationKind }) {
  if (kind === 'shift_validated') {
    return <Check size={16} color="#059669" strokeWidth={2.4} />;
  }
  if (kind === 'shift_rejected' || kind === 'worksite_rejected' || kind === 'worksite_shifts_cancelled') {
    return <X size={16} color="#DC2626" strokeWidth={2.4} />;
  }
  if (kind === 'worksite_assigned') {
    return <Building2 size={16} color="#2563EB" strokeWidth={2.2} />;
  }
  if (kind === 'worksite_approved') {
    return <Building2 size={16} color="#059669" strokeWidth={2.2} />;
  }
  return <Clock size={16} color={Colors.primary} strokeWidth={2.2} />;
}

function rowIconStyle(kind: CollaboratorNotificationKind) {
  switch (kind) {
    case 'shift_validated':
    case 'worksite_approved':
      return styles.rowIconSuccess;
    case 'shift_rejected':
    case 'worksite_rejected':
    case 'worksite_shifts_cancelled':
      return styles.rowIconDanger;
    case 'worksite_assigned':
      return styles.rowIconWorksite;
    default:
      return styles.rowIconTimesheet;
  }
}

export function CollaboratorNotificationBell({ variant = 'light' }: Props) {
  ...
  const handlePressItem = (item: CollaboratorNotification) => {
              ...
              >
                {items.map((item) => {
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.75}
                  >
                    <View style={[styles.rowIcon, rowIconStyle(item.kind)]}>
