...
| Tầng | Vai trò | Ví dụ |
|---|---|---|
| **Legacy Sources** | Input phân tích (không phải SoT runtime đích) | Frontend A/B, Supabase A/B, dumps, migrations |
| **Merge Specification** | **SoT thiết kế hợp nhất** (Phase 3) | Mapping schema/rules/auth/data |
| **Unified Domain Model** | SoT nghiệp vụ thống nhất | Bounded contexts, use-cases |
| **Unified Backend** | SoT runtime ứng dụng | Express modules / services |
| **Unified Database** | SoT persistence đích | PostgreSQL DDL + constraints |

**Supabase = Legacy Source only** sau khi Backend lên.  
**Không** dùng “Production SoT đơn lẻ = một project ref” làm đích kiến trúc.

Tài liệu RE hiện có trong `migration-analysis/` = **Legacy Analysis** (chủ yếu hệ thống đã reverse-engineer trong workspace). Xem `migration-analysis/README.md` (phân loại Legacy / Unified / Merge).
...
| System B (FE B / Supabase B) | ⬜ Chưa có trong workspace analysis | Gate đầu Phase 3: identify & ingest Legacy B |
| Phase 3+ | ⬜ Redesigned (Consolidation roadmap) | Không dùng roadmap migration 1:1 cũ |
| Backend scaffold `api-chantier/` | Partial / opportunistic | **Không** = Architecture Done |
| Hướng tiếp theo | **Phase 3 — Merge Specification** (sau khi có đủ Legacy A+B inputs) | Không nhảy Unified Backend Design khi Merge Spec chưa Done |
