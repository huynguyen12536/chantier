...
import { canReceiveApprovalNotifications, canViewTeamAbsences } from '@/utils/role';
...
export default function TeamAbsencesScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === 'today') {
      ...
      rows = rows.filter((row) => row.date_debut > todayKey);
    } else {
      rows = rows.filter((row) => row.date_fin < todayKey);
    }
  ...
  }, [absences, search, tab, periodTab, todayKey, weekBounds.end, weekBounds.start]);
  ...
  const mainTabs = useMemo(
    () =>
      [
        ...
        { key: 'history' as const, label: a.tabHistory, badge: 0, Icon: History },
      ],
    [a.tabToday, a.tabUpcoming, a.tabHistory, todayBadge, upcomingBadge],
  );
  ...
  if (!profile || !canViewTeamAbsences(profile.role)) return null;
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {loading ? (
          ...
          <Text style={styles.empty}>{a.emptyTeam}</Text>
        ) : (
          filtered.map((row) => (
            <TouchableOpacity
              key={row.id}
