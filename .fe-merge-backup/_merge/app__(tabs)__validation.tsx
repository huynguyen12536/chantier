...
import { canValidate, getHomeRouteForRole } from '@/utils/role';
...
export default function ValidationScreen() {
  ...
  const loadDeclarations = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      (declarations || []).forEach((rawDecl) => {
        const profileRow = rawDecl.profiles as DeclarationWithDetails['profiles'] | null | undefined;
        const chantierRow = rawDecl.chantiers as DeclarationWithDetails['chantiers'] | null | undefined;
        if (!profileRow || !chantierRow) {
          console.warn('[validation] skipping declaration with missing embed', rawDecl.id);
          return;
        }
  ...
  if (!profile?.role || !canValidate(profile.role)) {
  ...
  const renderDeclarationCard = (
    ...
    const showReason = Boolean(
      period?.commentaire
      && period.heure_fin
      && !declarationChantierBlocked(decl)
